...
public:
  ...
  CommsRelayTest()
  {
    ...
    pubToCommsPayload = node.advertise<seagull_commons_msgs::SeagullPayload>(
        seagull_communication::topics::TO_COMMS_RELAY_TOPIC, 1000);
    pubFromAPPayload = node.advertise<seagull_autopilot_msgs::AutopilotPayload>(
        seagull_autopilot::topics::AP_FROM_PAYLOAD_TOPIC, 1000);
    subToAPPayload = node.subscribe(seagull_autopilot::topics::AP_TO_PAYLOAD_TOPIC, 1000,
                                    &CommsRelayTest::ToAutopilotPayloadCallback, this);
    ...
    std::string file = "seagull_communication/test/image_640_480.jpg";
    ...
    {
      gen_random(picture, FRAME_SIZE_640_480);
...
protected:
  ...
  {
    this->saveImage("FromCommsRelayPayloadImage.jpg",reinterpret_cast<const char*>(&(msg->data[0])),msg->data.size());
  }
...
/**
 * Test the parser and serialization of each message chunk
 */
TEST_F(CommsRelayTest, testParser)
{
  CommsRelay node;
  ...
  data.push_back('A');
  RawData m;
  CommsUtils::serializeReliableMsg(1, &data[0], 0, data.size(), m, RMT_REQUIRES_ACK);
//std::cout << CommsUtils::toString(m) << "\n";
  ReliableMessage rm;
  bool b = false;
 ...
 *
 */
TEST_F(CommsRelayTest, testReceiveSeagullPayloadAck)
{

  TestReceiveSeagull(true);

  while (iAckCount <= (picture.size() / CHUNK_MAX_SIZE))
  {
    ros::spinOnce();
    rate->sleep();
  }
  saveImage("testReceiveSeagullPayloadAck.jpg", imageMap);
  ASSERT_EQ(true, bParsed);
}
/**
 *
 */
TEST_F(CommsRelayTest, testReceiveSeagullPayloadNoAck)
{
  return;
  gReceivedAutopilotPayload = false;
  TestReceiveSeagull(false);
  while (!gReceivedAutopilotPayload)
  {
    ros::spinOnce();
    rate->sleep();
  }
  saveImage("testReceiveSeagullPayloadNoAck.jpg", imageMap);
  ASSERT_EQ(true, bParsed);
}
/**
 *
 */
TEST_F(CommsRelayTest, testGroundStation2UAVComms)
{
  return;

  wait4Publishers(subToAPPayload);
  wait4Subscribers(pubFromAPPayload);

  uint16_t nrChunks = CommsUtils::calculateNrOfChunks(picture.size());

  for (int sequence = 0; sequence < nrChunks; ++sequence)
  {
    RawData m;
    CommsUtils::serializeReliableMsg(100, (uint8_t*)&picture[sequence*CHUNK_MAX_SIZE], sequence, picture.size(), m, RMT_REQUIRES_ACK);
    seagull_autopilot_msgs::AutopilotPayload apMsg;
    apMsg.header.vehicleId = 1;
    apMsg.data.assign(m.begin(), m.end());
    apMsg.len = m.size();
    usleep(10000);

    pubFromAPPayload.publish(apMsg);

  }

  while (iAckCount < (nrChunks))
  {
    ros::spinOnce();
    rate->sleep();
  }
...
TEST_F(CommsRelayTest, testGroundStation2UAVCommsIncomplete)
{
return;
 wait4Publishers(subToAPPayload);
 wait4Subscribers(pubFromAPPayload);
 ...
 for (int sequence = 0; sequence < nrChunks-1; ++sequence)
 {
   RawData m;
   CommsUtils::serializeReliableMsg(100, (uint8_t*)&picture[sequence*CHUNK_MAX_SIZE], sequence, picture.size(), m, RMT_REQUIRES_ACK);
...
{
  ros::M_string m;
  m["__master"] = "http://localhost:11311";
  ros::init(m, "test_CommsRelay");

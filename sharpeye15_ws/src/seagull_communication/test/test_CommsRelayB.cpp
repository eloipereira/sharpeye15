...
#define FRAME_SIZE_640_480 (307200/3)
...
public:
  ...
  CommsRelayTest()
  {
    ...
    pubToCommsPayload = node.advertise<seagull_communication_msgs::SeagullPayload>(
...
protected:
  ...
  void FromCommsRelayPayloadCallback(const seagull_communication_msgs::SeagullPayload::ConstPtr& msg)
  {
    ROS_INFO("Received data from Comms Relay");
    this->saveImage("FromCommsRelayPayloadImageONBoard.jpg", reinterpret_cast<const char*>(&(msg->data[0])),
                    msg->data.size());
  }
  ...
  void TestReceiveSeagull(bool reliable)
  {
    ...
    seagull_communication_msgs::SeagullPayload m;
...
TEST_F(CommsRelayTest, testUAVCommsSendReliableImage)
{
...
//  wait4Publishers(subFromCommsRealayPayload);
  ...
  seagull_communication_msgs::SeagullPayload pMsg;
  pMsg.header.vehicleId = 1;
  pMsg.data.assign(picture.begin(), picture.end());
  pMsg.requiresAck = true;
  pMsg.lenght = picture.size();
  usleep(10000);

  pubToCommsPayload.publish(pMsg);

  sleep(5);
  return;
}
TEST_F(CommsRelayTest, testUAVCommsSendUnReliableData)
{

  wait4Subscribers(pubToCommsPayload);

  uint16_t nrChunks = 1;//CommsUtils::calculateNrOfChunks(picture.size());
  seagull_communication_msgs::SeagullPayload pMsg;
  gen_random(pMsg.data,5);
  pMsg.header.vehicleId = 2050;
  pMsg.requiresAck = false;
  pMsg.lenght = pMsg.data.size();
  usleep(10000);
